...
class IsCustomerUser(permissions.BasePermission):
    """
    Permission class that allows customer users to modify resources.

    Safe methods (GET, HEAD, OPTIONS) are allowed for all users.
    Write methods require authenticated customer users or superusers.
    """
    ...
    def has_permission(self, request, user):
        ...
        if request.method in SAFE_METHODS:
            return True
        return bool(request.user.is_authenticated and (request.user.type == 'customer' or request.user.is_superuser))
